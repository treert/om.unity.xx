...
{
    [Tooltip("指定动态骨骼的根节点，其所有子节点自动定义为动态骨骼。")]
    public Transform m_Root = null;
    [Tooltip("内部物理模拟速率，每秒钟帧数。")]
    public float m_UpdateRate = 60.0f;
    [Range(0, 1)]
    [Tooltip("阻尼系数，子骨骼被带动时受到的阻力。")]
    public float m_Damping = 0.1f;
    [Tooltip("参数分布，参数在骨骼链上的变化，曲线值乘以参数值。")]
    public AnimationCurve m_DampingDistrib = null;
    [Range(0, 1)]
    [Tooltip("弹力，影响骨骼达到运算位置的速度，越大越快。")]
    public float m_Elasticity = 0.1f;
    [Tooltip("参数分布，参数在骨骼链上的变化，曲线值乘以参数值。")]
    public AnimationCurve m_ElasticityDistrib = null;
    [Range(0, 1)]
    [Tooltip("硬度，骨骼的硬度，值越小，越柔软。")]
    public float m_Stiffness = 0.1f;
    [Tooltip("参数分布，参数在骨骼链上的变化，曲线值乘以参数值。")]
    public AnimationCurve m_StiffnessDistrib = null;
    [Range(0, 1)]
    [Tooltip("惰性，忽略角色位置变化的程度。")]
    public float m_Inert = 0;
    [Tooltip("参数分布，参数在骨骼链上的变化，曲线值乘以参数值。")]
    public AnimationCurve m_InertDistrib = null;
    [Tooltip("半径，每节动态骨骼自身的物理碰撞半径。")]
    public float m_Radius = 0;
    [Tooltip("参数分布，参数在骨骼链上的变化，曲线值乘以参数值。")]
    public AnimationCurve m_RadiusDistrib = null;

    [Tooltip("末端长度，若不为0，则在转换动态骨骼链时在最后生成一个额外的骨骼，长度乘以最后两块骨头的距离。")]
    public float m_EndLength = 0;
    [Tooltip("末端偏移，若不为0，则在转换动态骨骼链时在最后生成一个额外的骨骼，并叠加角色局部空间的偏移量。")]
    public Vector3 m_EndOffset = Vector3.zero;
    [Tooltip("重力，这种力在世界空间中作用于骨骼的初始位置。")]
    public Vector3 m_Gravity = Vector3.zero;
    [Tooltip("力，这种力在世界空间中作用于骨骼的初始位置。")]
    public Vector3 m_Force = Vector3.zero;
    [Tooltip("设置物理碰撞，可防止动态骨骼的穿透。")]
    public List<DynamicBoneCollider> m_Colliders = null;
    [Tooltip("设置模拟排除，将选定的骨骼排除在动态模拟之外。")]
    public List<Transform> m_Exclusions = null;    
    public enum FreezeAxis
    {
        None, X, Y, Z
    }
    [Tooltip("轴冻结，可设置骨头在制定平面上模拟。")]
    public FreezeAxis m_FreezeAxis = FreezeAxis.None;
    [Tooltip("启用距离禁用，开启后，角色与相机一定距离外会停止物理模拟。")]
    public bool m_DistantDisable = false;
    [Tooltip("制定相机，不指定为默认当前相机。")]
    public Transform m_ReferenceObject = null;
    [Tooltip("设置停止物理模拟的距离，角色与相机的直线距离。")]
